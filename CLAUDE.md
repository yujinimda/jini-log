...
**`description`은 검색 결과에 그대로 노출된다.** 목록 카드용 요약이 아니라 **검색 유입의 입구**다.

- **이 글을 찾는 사람이 검색창에 칠 말**을 넣는다. 제목이 은유적일수록(「가리키는 쪽이 남의 번호를 든다」) 여기서 받쳐줘야 한다 — 제목은 그대로 두고 `description`만 검색어를 담는 쪽이 안전하다.
- 구체적인 이름을 쓴다. "외래키" 말고 `MySQL 외래키 FOREIGN KEY`, "예외 처리" 말고 `printStackTrace 종료코드`.
- 80~130자를 목표로 한다. 200자가 상한이고, 너무 짧으면 키워드가 안 들어간다. 에디터가 80자 미만이면 부드럽게 알려준다(차단은 아님).
- 감상("~를 알게 됐습니다")으로 끝내지 말고 **글에서 다루는 것**을 나열한다. 감상은 본문에서 한다.

**사이트맵·RSS·색인 제출은 손댈 필요 없다.** `app/sitemap.ts`가 빌드마다 `content/posts`를 통째로 다시 읽으므로 발행 = 재배포 = 자동 갱신이다. Search Console에 사이트맵 주소를 한 번 제출한 것이 계속 유효하다. 급할 때만 URL 검사에서 색인을 개별 요청한다.
