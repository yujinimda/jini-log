...
import { auth, isOperator, isOperatorSession } from "@/lib/auth";
...
vi.mock("@/lib/auth", () => ({
  auth: vi.fn(),
  isOperator: vi.fn(),
  isOperatorSession: vi.fn(), // 라우트 가드(requireOperator)가 사용
}));
...
function setAuth(session: unknown, operator: boolean) {
  ...
  vi.mocked(isOperatorSession).mockReturnValue(operator);
}
...
/** Octokit은 path를 통째로 인코딩(%2F 포함) — 와일드카드로 받고 디코딩해서 분기한다 */
function decodedContentPath(url: string) {
  return decodeURIComponent(
    new URL(url).pathname.replace("/repos/test-owner/test-repo/contents/", ""),
  );
}

function stubContents(map: Record<string, () => Response>) {
  server.use(
    http.get(`${githubApi}/contents/*`, ({ request }) => {
      const handler = map[decodedContentPath(request.url)];
      return handler ? handler() : HttpResponse.json({ message: "Not Found" }, { status: 404 });
    }),
  );
}
...
function stubContentsListing() {
  stubContents({
    "content/posts": () =>
      HttpResponse.json([
        directoryItem("hello-world.mdx", "content/posts/hello-world.mdx", "post-sha"),
      ]),
    "content/drafts": () =>
      HttpResponse.json([
...
describeListRoute("POST /api/admin/posts dashboard actions", () => {
  it("발행취소는 posts에서 drafts로 이동하는 단일 Git Data API 커밋을 만든다", async () => {
    ...
    stubContents({
      "content/posts/hello-world.mdx": () =>
        HttpResponse.json(
          encodedFile(publishedSource, "published-sha", "content/posts/hello-world.mdx"),
        ),
    });
    server.use(
      // ref 경로도 단일 인코딩 세그먼트(heads%2Fmain)로 온다 — 와일드카드 매칭
      http.get(`${githubApi}/git/ref/*`, () =>
      ...
      http.patch(`${githubApi}/git/refs/*`, () =>
        HttpResponse.json({ object: { sha: "unpublish-commit-sha" } }),
      ),
      http.patch(`${githubApi}/git/ref/*`, () =>
        HttpResponse.json({ object: { sha: "unpublish-commit-sha" } }),
      ),
    );
  ...
  it("삭제는 sha 확인 후 파일 제거 커밋을 만들고 deleted 상태를 반환한다", async () => {
    ...
    server.use(
      http.delete(`${githubApi}/contents/*`, async ({ request }) => {
