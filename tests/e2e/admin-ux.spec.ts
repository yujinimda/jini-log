// 어드민 UX e2e — 앱 내 확인 다이얼로그·토스트·정렬 (spec US3, FR-012·013·014, T025).
// 레인 D(lane/002-d-tests)에서 작성됐으나 main에 머지되지 않아 그동안 실행되지 않았다.
// 레인 C 구현이 끝나 파일 상단의 조건부 skip은 전부 해제된 상태로 6케이스 모두 실행된다.
//
// 파괴적 액션 안전장치: delete 액션 요청은 page.route로 가로채 fulfill하므로
// 실제 /api/admin/posts·GitHub 콘텐츠 레포에 도달하지 않는다.
...
async function confirmDeleteWithRoute(
  ...
  const dialog = await openDeleteDialog(page, "hello-world");
