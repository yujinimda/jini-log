...
- 레일 2종은 **xl(1280px) 이상에서만** 표시하고 그 미만은 숨긴다 — 필요 폭 `2 × (24 + 2 + 12) = 76rem`.
  - 좌: `<PostSidebar>` 전체 발행 글 (`nav[aria-label="전체 글"]`, 현재 글은 `aria-current="page"`)
    - 변경 이력: **분류 → 글 2단** (spec.md [C13](../spec.md#후속-변경-이력), 2026-07-29). 위계는 `전체 글`(h2) > 분류(h3) > 글(`ul>li>a`).
      - **분류 헤더는 링크가 아니다** — `/categories` 페이지를 두지 않는다. 따라서 "레일의 링크 수 = 발행 글 수"가 계약으로 유지된다.
