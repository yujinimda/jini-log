...
- 레일 2종은 **xl(1280px) 이상에서만** 표시하고 그 미만은 숨긴다 — 필요 폭 `2 × (24 + 2 + 12) = 76rem`.
  - 좌: `<PostSidebar>` 전체 발행 글 (`nav[aria-label="전체 글"]`, 현재 글은 `aria-current="page"`)
    - 변경 이력: **분류 → 글 2단** (spec.md [C13](../spec.md#후속-변경-이력), 2026-07-29). 위계는 `전체 글`(h2) > 분류(summary) > 글(`ul>li>a`).
    - 변경 이력: **접기 + 분류 페이지** (spec.md [C14](../spec.md#후속-변경-이력), 2026-08-21). 레일은 전수 목록이 아니라 **요약**이다:
      - 기본 전부 접힘(`<details>` — 분류명+개수만) · 글 상세에서는 현재 글의 분류만 자동 열림 · 열린 분류는 **최신 5개**(+ 현재 글이 그 밖이면 6번째로) · 6개 초과 분류는 "전체 N개 →"(`/categories/[category]`).
      - ~~"레일의 링크 수 = 발행 글 수"~~ 계약 **폐기** — 접힌 details 안의 링크는 접근성 트리에 없다. 전체 목록의 단일 출처는 분류 페이지다.
      - 분류 페이지는 태그 페이지와 동일 계약(SSG·`dynamicParams=false`·sitemap 포함 — C13의 "sitemap 불필요" 판단을 뒤집음). 연도 묶음 + 날짜·제목·description 압축 목록.
      - 어느 분류를 열지는 클라이언트 몫(공유 레이아웃은 현재 경로를 서버에서 모른다). 여닫기 자체는 `<details>`라 JS 불필요.
      - 그룹핑은 서버(`app/(blog)/layout.tsx`)에서 `groupPostsByCategory()`로 끝낸다. 클라이언트 컴포넌트가 받는 것은 `{category, posts:[{slug,title}]}[]`뿐 — 본문·발췌·날짜는 번들로 넘기지 않는다.
      - 순서: 분류는 그 분류의 최신 글 날짜 내림차순, 분류 안에서도 날짜 내림차순. 동률은 각각 분류명·slug 오름차순으로 깬다. **SSG 출력이 빌드마다 흔들리지 않도록 결정적이어야 한다** — 그래서 기본 locale에 의존하는 무인자 `localeCompare()`를 쓰지 않는다.
